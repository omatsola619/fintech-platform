...
const MOCK_LOGS = [
    { id: '1', time: '14:32:01.041', event: 'Request Sent', provider: 'Paystack', status: 'Pending', ref: 'tx_pay_90210a' },
    { id: '2', time: '14:32:01.282', event: 'Response Received', provider: 'Paystack', status: 'Success', ref: 'tx_pay_90210a' },
    { id: '3', time: '14:32:02.100', event: 'Webhook Received', provider: 'Paystack', status: 'Success', ref: 'tx_pay_90210a' },
    { id: '4', time: '14:30:10.111', event: 'Request Sent', provider: 'Flutterwave', status: 'Pending', ref: 'tx_flw_10293b' },
    { id: '5', time: '14:30:11.311', event: 'Response Received', provider: 'Flutterwave', status: 'Failed', ref: 'tx_flw_10293b' },
];
    ...
    return (
        <div className="max-w-6xl mx-auto flex flex-col h-[calc(100vh-6rem)] animate-in fade-in slide-in-from-bottom-4 duration-500 overflow-hidden relative">
            ...
            <div className="shrink-0 space-y-4 mb-6">
                <div>
                    <h1 className="font-['Outfit'] text-2xl sm:text-3xl font-bold text-black mb-1">Developer Logs</h1>
                    <p className="text-slate-600 text-sm">Deep dive into provider requests, responses, and webhook events.</p>
                </div>
                        ...
                        <tbody className="divide-y divide-slate-100 text-[13px]">
                            {MOCK_LOGS.map((log) => (
                                <tr key={log.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-6 py-4 text-blue-600 font-medium">{log.event}</td>
                                    <td className="px-6 py-4 text-slate-900 font-sans">{log.provider}</td>
                                    <td className="px-6 py-4 text-slate-500">{log.ref}</td>
                                    <td className="px-6 py-4">
                                        <span className={`inline-flex items-center gap-1 font-bold tracking-wider px-2 py-0.5 rounded text-[10px] uppercase font-sans ${log.status === 'Success' ? 'bg-emerald-100 text-emerald-700' : log.status === 'Failed' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>
                                            {log.status}
                                        </span>
                                    </td>
                    ...
                    <div className="relative w-full max-w-2xl bg-[#0d1117] rounded-2xl shadow-2xl flex flex-col transform transition-transform duration-300 animate-in fade-in zoom-in-95 overflow-hidden">
                        ...
                        <div className="px-4 py-3 border-b border-slate-800 flex items-center justify-between bg-[#161b22]">
                            <div className="flex items-center gap-2">
                                ...
                                <h2 className="font-semibold text-slate-200 text-sm">{selectedLog.event}</h2>
                                <span className="text-xs font-mono text-slate-500 ml-2">({selectedLog.ref})</span>
                            </div>
                            <div className="flex items-center gap-2">
                                ...
                                <button onClick={() => setSelectedLog(null)} className="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-800 rounded-lg transition-colors cursor-pointer ml-1">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        </div>
